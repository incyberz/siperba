...
$sql_peserta = "SELECT p.id AS peserta_id, e.nama_event, e.batas_pengumpulan
    FROM tb_peserta p
    JOIN tb_user u ON u.id = p.user_id
    JOIN tb_event e ON e.id = p.event_id
    WHERE u.username = '$username' 
    AND e.batas_pengumpulan < now()
      ...
      <div class="row row-cols-1 row-cols-md-2 g-3">
        <?php while ($row = mysqli_fetch_assoc($peserta_res)):
          ...
          if (mysqli_num_rows($pengumpulan_res) > 0) {
            ...
            $status_badge = "<span class='badge bg-secondary'>Anda " . ($isClosed ? 'Tidak' : 'Belum') . " Upload</span>";
          }
          ...
          $info_berkas = $file_link ? "<p class='card-text'><strong>Berkas:</strong> $file_link</p>" : '';
